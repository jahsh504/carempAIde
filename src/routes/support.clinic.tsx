    ...
    return (
      <div className="px-4 pb-6 space-y-4">
        <div className={cn("rounded-3xl p-5 text-white bg-gradient-to-br", a.grad)}>
          ...
          <p className="mt-3 text-center text-[11px] uppercase tracking-wider opacity-90">
            Appointment held
          </p>
          <h1 className="mt-1 text-center text-xl font-semibold">{clinic.name}</h1>
          <p className="mt-1 text-center text-[13px] opacity-90">
            {slot} · Today · {clinic.specialty}
          </p>
        </div>
        ...
        <div className="relative overflow-hidden rounded-3xl border border-border">
          <div className="h-44 bg-[linear-gradient(135deg,oklch(0.94_0.02_200),oklch(0.96_0.01_220))] relative">
            <svg className="absolute inset-0 h-full w-full opacity-70" viewBox="0 0 400 200">
              <path
                d="M0 120 Q100 90 200 110 T400 100"
                stroke="var(--teal)"
                strokeWidth="2"
                fill="none"
                strokeDasharray="4 4"
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-[12px] text-muted-foreground">
                {clinic.address} · ~{clinic.distance} · 12 min by car
              </p>
            </div>
          </div>
          <button
            className={cn("mt-3 w-full rounded-full py-2.5 text-sm font-semibold text-white", a.bg)}
          >
        ...
        <Card>
          ...
          <ul className="mt-2 space-y-1.5 text-sm">
            <li className="flex items-start gap-2">
              <Check className="mt-0.5 h-4 w-4 text-emerald" /> Government ID
            </li>
            <li className="flex items-start gap-2">
              <Check className="mt-0.5 h-4 w-4 text-emerald" /> Previous reports (auto-shared from
              records)
            </li>
            <li className="flex items-start gap-2">
              <Check className="mt-0.5 h-4 w-4 text-emerald" /> Arrive 10 min early for paperwork
            </li>
          </ul>
        </Card>
      ...
      <div className="px-4 pb-6 space-y-4">
        ...
        <Card>
          ...
          <div className="mt-2 flex items-center gap-2 text-[12px]">
            <Star className="h-3.5 w-3.5 fill-amber text-amber" /> {clinic.rating} · Consultation ₹
            {clinic.fee}
          </div>
        </Card>
    ...
    <div className="px-4 pb-6 space-y-4">
      <div className={cn("rounded-3xl p-4 text-white bg-gradient-to-br", a.grad)}>
        <div className="flex items-center gap-2 text-[11px] uppercase tracking-wider opacity-90">
          <Hospital className="h-3.5 w-3.5" /> Clinics near you
        </div>
        <h1 className="mt-1 text-lg font-semibold">Book a clinic visit</h1>
        <p className="mt-1 text-[12px] opacity-90">
          Live slot availability · 4 clinics within 5 km
        </p>
      </div>
      <div className="space-y-2.5">
        {clinics.map((c) => (
          ...
          >
            <div
              className={cn(
                "grid h-14 w-14 place-items-center rounded-2xl text-white bg-gradient-to-br",
                a.grad,
              )}
            >
              <Hospital className="h-6 w-6" />
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="truncate text-[11px] text-muted-foreground">
                {c.specialty} · {c.distance}
              </p>
              <p className="text-[11px] text-muted-foreground">
                <Star className="mr-0.5 inline h-3 w-3 fill-amber text-amber" /> {c.rating} · ₹
                {c.fee}
              </p>
            </div>
