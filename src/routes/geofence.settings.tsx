...
export const Route = createFileRoute("/geofence/settings")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Control indoor tracking, exit alerts, caregiver notifications, scan frequency and safe radius." },
      { property: "og:title", content: "Geofence Settings — careMP AIDE" },
      { property: "og:description", content: "Tune indoor tracking and alerting to suit your household." },
...
const toggles = [
  ...
  { id: "caregiver", label: "Enable Caregiver Notifications", hint: "Share alerts with the care team" },
];
    ...
    <div className="flex rounded-full border border-border bg-card p-1">
      {options.map((o) => (
        <button
          key={o}
          onClick={() => onChange(o)}
          className={cn("flex-1 rounded-full py-1.5 text-[12px] font-medium transition-colors", value === o ? "bg-primary text-primary-foreground" : "text-muted-foreground")}
        >
          {o}
        </button>
      ))}
...
function GeofenceSettingsPage() {
  const [on, setOn] = useState<Record<string, boolean>>({ tracking: true, exit: true, caregiver: true });
  ...
  return (
    <div className="space-y-5 px-4 pb-6">
      <div className="px-1 pt-1">
        ...
        <p className="mt-1.5 text-[12.5px] leading-relaxed text-muted-foreground">Tune how closely careMP watches, and who hears about it.</p>
      </div>

      <Card className="p-0">
        {toggles.map((t, i) => (
          <div key={t.id} className={cn("flex items-center gap-3 px-4 py-3.5", i < toggles.length - 1 && "border-b border-border")}>
