...
import {
  Search, MapPin, Siren, Video, Apple, Salad, Brain, Sparkles, Hospital,
  HeartPulse, Activity, HandHeart, TestTube, Pill, Sparkle, X, ChevronRight,
} from "lucide-react";
import { onlineServices, offlineServices, aiRecommendations, accentClass, type Accent } from "@/data/health-circle";
...
export const Route = createFileRoute("/support")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "On-demand online consultations and offline care — booked in seconds, powered by your health data." },
    ],
  }),
  component: Support,
});
...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  video: Video, apple: Apple, salad: Salad, brain: Brain, sparkles: Sparkles,
  hospital: Hospital, "heart-pulse": HeartPulse, activity: Activity, "hand-heart": HandHeart,
  "test-tube": TestTube, pill: Pill, siren: Siren,
};
...
function offlineHref(key: string) {
  switch (key) {
    case "clinic":    return { to: "/support/clinic" as const };
    case "lab":       return { to: "/support/lab" as const };
    case "pharmacy":  return { to: "/support/pharmacy" as const };
    case "ambulance": return { to: "/support/ambulance" as const };
    default:          return { to: "/support/home-care/$service" as const, params: { service: key } };
  }
}
  ...
  return (
    <div className="px-4 pb-6 space-y-4">
      ...
      <div className="flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2.5">
        ...
        <input placeholder="Search services, doctors, tests…" className="w-full bg-transparent text-sm outline-none" />
      ...
      <Link to="/support/ambulance" className="block">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-coral to-amber p-4 text-white soft-shadow">
          <div className="flex items-center gap-3">
            ...
            <div className="rounded-full bg-white/25 px-3 py-1.5 text-xs font-semibold">Request</div>
          </div>
        </div>
      </Link>
        ...
        <div>
          ...
          <div className="no-scrollbar -mx-4 flex gap-3 overflow-x-auto px-4 pb-1">
            {recs.map((r) => {
              ...
              return (
                <div key={r.id} className={cn("relative w-[260px] shrink-0 card-surface p-3 rise-in border")}>
                  <button
                    aria-label="Dismiss"
                  ...
                  <div className="flex items-center gap-2">
                    <div className={cn("grid h-8 w-8 place-items-center rounded-xl", a.soft, a.text)}>
                      <Sparkle className="h-4 w-4" />
                    </div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">AI suggestion</p>
                  </div>
                  <p className="mt-2 text-[13px] leading-snug">{r.reason}</p>
                  {r.attached && (
                    <p className={cn("mt-1 inline-block rounded-full px-2 py-0.5 text-[10px]", a.soft, a.text)}>📎 {r.attached}</p>
                  )}
                  <Link {...(href as any)} className={cn("mt-3 inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-[11px] font-semibold text-white", a.bg)}>
      ...
      <div className="rounded-full border border-border bg-card p-1 grid grid-cols-2 text-sm font-medium">
        {(["online", "offline"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={cn("rounded-full py-2 transition-colors capitalize",
              tab === t ? "bg-primary text-primary-foreground soft-shadow" : "text-muted-foreground")}
          >
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            {onlineServices.map((s) => {
              ...
              return (
                <Link key={s.key} to="/support/online/$service" params={{ service: s.key }} className="card-surface p-4 rise-in">
            ...
            {offlineServices.map((s) => {
              ...
              return (
                <Link key={s.key} {...(href as any)} className="card-surface p-4 rise-in">
                  ...
                  <div className="mt-2 flex items-center justify-between text-[11px]">
                    <span className="text-emerald">{s.eta}</span>
                    <span className="text-muted-foreground">{s.from ? `from ₹${s.from}` : "Free delivery"}</span>
                  </div>
                </Link>
              );
            })}
