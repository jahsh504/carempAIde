...
export const Route = createFileRoute("/reports/$id")({
  head: () => ({
    meta: [
      { title: `${reportDetail.title} — careMP` },
      { name: "description", content: `AI-summarized ${reportDetail.title}.` },
    ],
  }),
          ...
          return (
            <Card key={v.label}>
                ...
                <div className="flex items-baseline gap-1">
                  <span
                    className={cn(
                      ...
                      v.flag === "high" ? "text-amber" : "text-emerald",
                    )}
                  >
                    {v.value}
                  </span>
              ...
              <div className="relative mt-3 h-2 rounded-full bg-muted">
                <div
                  className="absolute inset-y-0 rounded-full bg-emerald/25"
                  style={{ left: `${okStart}%`, width: `${okEnd - okStart}%` }}
                />
                <div
                  className={cn(
                    "absolute -top-1 h-4 w-1 rounded-full",
                    v.flag === "high" ? "bg-amber" : "bg-emerald",
                  )}
                  style={{ left: `calc(${pct}% - 2px)` }}
                />
              </div>
              <p className="mt-2 text-[11px] text-muted-foreground">
                Reference: {v.range[0]}–{v.range[1]} {v.unit}
              </p>
            </Card>
          );
        })}
