  ...
  head: () => ({
    meta: [
      { title: "Refill Reminders — Medication inventory | careMP AIDE" },
      {
        name: "description",
        content:
          "See how many tablets are left for each medication, when they run out, and record refills.",
      },
      { property: "og:title", content: "Refill Reminders — Medication inventory | careMP AIDE" },
      {
        property: "og:description",
        content: "Never run out — track remaining tablets and estimated finish dates.",
      },
          ...
          return (
            <Card key={i.id}>
              <div className="flex items-baseline justify-between gap-3">
                ...
                <span className="num shrink-0 text-[12.5px] font-medium text-muted-foreground">
                  {i.strength}
                </span>
              </div>
              ...
              <div className="mt-4 flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-[15px] font-semibold">
                    <span className="num">{i.remaining}</span> Tablets
                  </p>
                </div>
              ...
              <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-muted">
                <div
                  className={cn(
                    "h-full rounded-full",
                    low ? "bg-amber" : "bg-gradient-to-r from-emerald to-teal",
                  )}
                  style={{ width: `${pct}%` }}
                />
              </div>
