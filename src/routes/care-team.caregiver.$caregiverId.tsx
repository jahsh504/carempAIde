...
export const Route = createFileRoute("/care-team/caregiver/$caregiverId")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Experience, certifications, reviews, availability and your Digital Twin compatibility match for this caregiver." },
      { property: "og:title", content: "Caregiver Profile — AI compatibility match | careMP AIDE" },
      { property: "og:description", content: "See why your Digital Twin suggests this caregiver, then send a care request." },
  ...
  if (!caregiver) {
    return (
      <div className="px-4 pb-6">
        <Card><p className="text-[13px] text-muted-foreground">This caregiver is no longer listed.</p></Card>
      </div>
    );
  }
    ...
    <div className="space-y-5 px-4 pb-6">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-twin p-5 text-twin-foreground soft-shadow rise-in">
        ...
        <div className="relative flex items-center gap-4">
          <div className={cn("grid h-20 w-20 place-items-center rounded-3xl bg-gradient-to-br text-xl font-semibold text-white", avatarTone[caregiver.tone])}>
            {caregiver.initials}
          </div>
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px] text-white/85">
              <span className="inline-flex items-center gap-1"><Star className="h-3 w-3 fill-current text-amber" /> {caregiver.rating} · {caregiver.reviews} reviews</span>
              <span className="inline-flex items-center gap-1"><Award className="h-3 w-3" /> {caregiver.experienceYears} yrs</span>
            </div>
          </div>
        </div>
      ...
      <Card className="border-teal/20 bg-gradient-to-br from-teal/5 to-blue/5">
        ...
        <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-muted">
          <div className="h-full rounded-full bg-gradient-to-r from-teal to-blue transition-all duration-700" style={{ width: `${caregiver.matchScore}%` }} />
        </div>
      ...
      <div>
        ...
        <Card>
          ...
          <div className="mt-3 space-y-2 border-t border-border pt-3 text-[11.5px] text-muted-foreground">
            <p className="flex items-start gap-2"><Languages className="mt-px h-3.5 w-3.5 shrink-0" /> {caregiver.languages.join(" · ")}</p>
            <p className="flex items-start gap-2"><BadgeCheck className="mt-px h-3.5 w-3.5 shrink-0" /> {caregiver.certifications.join(" · ")}</p>
            <p className="flex items-start gap-2"><MapPin className="mt-px h-3.5 w-3.5 shrink-0" /> {caregiver.distanceKm} km away</p>
            <p className="flex items-start gap-2"><Timer className="mt-px h-3.5 w-3.5 shrink-0" /> Can arrive in about {caregiver.etaMinutes} min</p>
          </div>
      ...
      <div>
        ...
        <Card>
          <label className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Reason for assistance</label>
          <select
            value={reason}
          ...
          >
            {assistanceReasons.map((r) => <option key={r} value={r}>{r}</option>)}
          </select>

          <label className="mt-4 block text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Notes (optional)</label>
          <textarea
            value={notes}
