  ...
  head: () => ({
    meta: [
      { title: "Assigned Caregivers — Your care team | careMP AIDE" },
      {
        name: "description",
        content:
          "Everyone assigned to your care — role, experience, languages, specialties and current shift.",
      },
      { property: "og:title", content: "Assigned Caregivers — Your care team | careMP AIDE" },
      {
        property: "og:description",
        content: "Meet the caregivers looking after you, with shifts and specialties.",
      },
  ...
  return (
    <div className="space-y-3 px-4 pb-6">
      <SectionHeader
        ...
        hint={`${caregivers.length} people on your care team`}
      />
      {caregivers.map((c) => (
        <Card key={c.id}>
          <div className="flex items-center gap-3.5">
            <div className="relative">
              ...
              {c.online && (
                <span className="absolute -bottom-1 -right-1 h-3.5 w-3.5 rounded-full border-2 border-card bg-emerald" />
              )}
            </div>
          ...
          <div className="mt-3 space-y-1.5 border-t border-border pt-3 text-[11px] text-muted-foreground">
            <p className="inline-flex items-center gap-1.5">
              <Award className="h-3 w-3" /> {c.experience} experience
            </p>
            <p className="inline-flex items-center gap-1.5">
              <Languages className="h-3 w-3" /> {c.languages.join(" · ")}
            </p>
            <p className="flex items-start gap-1.5">
              <Stethoscope className="mt-px h-3 w-3 shrink-0" />{" "}
              <span>{c.specialties.join(" · ")}</span>
            </p>
          </div>
