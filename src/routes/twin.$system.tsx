...
export const Route = createFileRoute("/twin/$system")({
  head: ({ params }) => {
    ...
    return {
      meta: [
        { title: `${s?.label ?? "System"} — Twin` },
        {
          name: "description",
          content: `${s?.label ?? "Body system"} details in your Digital Twin.`,
        },
      ],
    };
  },
    ...
    <div className="px-4 pb-6 space-y-4">
      <Card>
        ...
        <div className="mt-3 flex gap-1.5">
          {["Day", "Week", "Month", "Year"].map((t, i) => (
            <button
              key={t}
              className={`flex-1 rounded-full px-2 py-1.5 text-[11px] font-medium ${i === 1 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
            >
              {t}
            </button>
          ))}
      ...
      <Card className="border-teal/20 bg-gradient-to-br from-teal/5 to-blue/5">
        ...
        <p className="mt-2 text-sm">
          Your {s.label.toLowerCase()} score has climbed steadily since starting a consistent
          morning routine. Protect your recovery this weekend to lock in the gains.
        </p>
      </Card>
      ...
      <div>
        ...
        <div className="space-y-2">
          {[
            "Add a 10-minute breathwork session before bed",
            "Increase soluble fiber (oats, lentils, apples)",
            "Keep your Sunday recovery ritual — it's working",
          ].map((r, i) => (
